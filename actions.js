...
export function getActionDefinitions(self) {
	return {
		...
		sendcameracommand: {
			...
			options: [
				...
				// NEW: optional camera number
				{
					...
					tooltip:
						'If set, the command goes to that camera number no matter which camera is currently selected in Middle Control or Companion. If left empty, the command is sent to the currently selected (active) camera.',
					default: '',
					width: 6,
				},
			],
		...
		sendgimbalcommand: {
			...
			callback: async (event) => {
				const baseCmd = unescape(await self.parseVariablesInString(event.options.id_sendgimbalcommand))
		...
		preset: {
			...
			callback: async (event) => {
				...
				if (cmd === null) {
					self.log(
						'warn',
						'Preset: camera/preset number must be numeric (got ' + PresetNumber + ' / ' + PresetCamID + ')'
					)
					return
				}
		...
		// Action that stops the running preset (matches the app's Stop button)

		stoppreset: {
			name: 'Stop Running Preset',
			options: [
				{
					type: 'static-text',
					id: 'Textlabel',
					label:
						'Stops the running preset. With Camera ID empty this matches the Stop button in Middle Control: it stops the currently active camera, or every camera when the app is in All-Cameras mode. Set a Camera ID to stop the preset on that specific camera instead.',
					width: 12,
				},
				{
					type: 'textinput',
					id: 'id_stoppreset_camera',
					label: 'Camera ID (optional):',
					tooltip:
						'If set, stops the preset on that camera number regardless of the active selection. If left empty, stops the current camera (or all cameras in All-Cameras mode).',
					default: '',
					width: 6,
					regex: '^[0-9]*$',
					regexMessage: 'Only numbers are allowed',
		...
		setspeed: {
			...
			callback: async (event) => {
				...
				const camNum = unescape(await self.parseVariablesInString(event.options.id_setspeed_camera ?? '')).trim()
			...
			options: [
				{
					...
					id: 'Textlabel',
					label:
						'Set a custom absolute PTZ value for the Pan, Tilt, Roll & Zoom of your Gimbal. Leave the fields that you do not want to touch empty. ',
					width: 6,
				},
				{
					type: 'textinput',
